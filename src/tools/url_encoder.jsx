        ...
        <div className="flex gap-2">
          {['encode', 'decode'].map(m => (
            <button key={m} onClick={() => { setMode(m); setOutput(''); setError('') }}
              className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === m ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/30' : 'bg-white/[0.06] text-slate-400 border border-white/8'}`}>
