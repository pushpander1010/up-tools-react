            ...
            <div className="flex gap-2">
              {[{ v: 20, l: 'T20 (20 ov)' }, { v: 50, l: 'ODI (50 ov)' }, { v: 90, l: 'Test (90 ov)' }].map(f => (
                <button key={f.v} onClick={() => setFormat(f.v)}
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${format === f.v ? 'bg-indigo-500/15 text-indigo-400 border border-indigo-500/30' : 'bg-white/[0.06] text-slate-400 border border-white/[0.08]'}`}>
                  {f.l}
                </button>
              ))}
            </div>
