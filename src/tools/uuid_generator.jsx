    ...
    >
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex gap-2">
            {[['standard', 'Standard'], ['uppercase', 'UPPERCASE'], ['no-dash', 'No Dashes']].map(([val, label]) => (
              <button key={val} onClick={() => setFormat(val)}
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${format === val ? 'bg-brand/15 text-brand-light border border-brand/30' : 'bg-white/[0.03] text-slate-500 border border-white/6'}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2">
            {[1, 5, 10, 25].map(n => (
              <button key={n} onClick={() => setCount(n)}
                className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${count === n ? 'bg-brand/15 text-brand-light border border-brand/30' : 'bg-white/[0.03] text-slate-500 border border-white/6'}`}>
                {n}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-2">
          ...
          {uuids.map((u, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/6 hover:border-white/12 transition-all group">
